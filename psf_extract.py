...
#### BASIC PARAMETERS ##########
pixel_size = 0.6        # Pixel size of Spitzer IRAC CH1
irac_fwhm = 1.6         # Initial guess of FWHM of Spitzer IRAC CH1 data

num_psf_stars = 10      # Number of PSF stars
...
################################################################
class_range = 0.7       # Maximum value of CLASS_STAR selecting PSF stars form sex catalog
...
for x in range(126,len(sha_cat)):
#for x in range(0,1):
	fn = sha_cat['id'][x]+'.fits'		# Image file name
	shutil.copy(data_dir+'SHA_NGC_IC_LONG/'+fn,work_dir)
	...
	for i in range(min(num_psf_stars,len(stars))):
		...
		if len(ind)==0 or np.isnan(fits_data[int(stars['Y_IMAGE'][i] - psf_radius * nan_check) : int(stars['Y_IMAGE'][i]+psf_radius * nan_check ), \
        	        int(stars['X_IMAGE'][i] - psf_radius * nan_check) : int(stars['X_IMAGE'][i] + psf_radius * nan_check)]).any():
			continue
	...
	shutil.copy(fn[:-5]+'.psf1.fits',data_dir+'psfs')
	shutil.copy(fn[:-5]+'.psf2.fits',data_dir+'psfs')
	shutil.copy(fn[:-5]+'.psf3.fits',data_dir+'psfs')

	os.system('rm test.cat')
#	os.system('rm bkg_*.fits')
#	os.system('rm '+fn[:-5]+'*')
